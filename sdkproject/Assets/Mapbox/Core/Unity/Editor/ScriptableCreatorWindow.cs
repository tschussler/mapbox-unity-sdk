namespace Mapbox.Editor
{
    ...
    public class ScriptableCreatorWindow : EditorWindow
    {
        ...
        public static void Open(Type type, SerializedProperty p, int index = -1)
        {
            var window = GetWindow<ScriptableCreatorWindow>(true, "Do your thing!");
            window._type = type;
            window._finalize = p;
        ...
        void OnGUI()
        {
            ...
            for (int i = 0; i < _assets.Length; i++)
            {
                ...
                if (_showElement[i])
                {
                    ...
                    GUI.enabled = false;
                    var ed = Editor.CreateEditor(asset);
