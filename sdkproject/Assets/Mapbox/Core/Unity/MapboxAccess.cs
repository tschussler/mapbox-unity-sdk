...
namespace Mapbox.Unity
{
	...
	public class MapboxAccess : IFileSource
	{
		...
		Dictionary<string, Action<Response>> _cachedRequests = new Dictionary<string, Action<Response>>();
		...
		void SendCachedRequests()
		{
			foreach (var request in _cachedRequests)
			{
				...
				Debug.Log("MapboxAccess: " + "Request sent: " + request.Key);
			}
		...
		public IAsyncRequest Request(string url, Action<Response> callback)
		{
			...
			{
				if (!_cachedRequests.ContainsKey(url))
				{
					_cachedRequests.Add(url, callback);
					Debug.Log("MapboxAccess: " + "No token, waiting . . . ");
					return new CachedAsyncRequest();
				}
			}

			Debug.Log("MapboxAccess: " + "Immediate request: " + url);
		...
		class CachedAsyncRequest : IAsyncRequest
		{
			public void Cancel()
			{
				//TODO: bubble this to the real request.
			}
		}
